...
```{r}
data(qol_cancer)
result <- describe_distribution(qol_cancer$age)
result
...
plot(result, highlight = c("D", "H"), size = .4)

# own color scales - pass a named vector to 'scale_fill_manual()' 
# the name of the non-highlighted color is "no_highlight".
plot(result, highlight = c("D", "H", "A"), size = .4) + ggplot2::scale_fill_manual(values = c(D = "red", H = "green", A = "gold", no_highlight = "steelblue"))
```
