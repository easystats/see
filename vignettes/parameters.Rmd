...
`parameters_simulate()` computes simulated draws of parameters and their related indices such as Confidence Intervals (CI) and p-values. Simulating parameter draws can be seen as a (computationally faster) alternative to bootstrapping.

As `parameters_simulate()` is based on `model_simulate()` and thus simulates many draws for each parameter, `plot()` will produce similar plots as the density estimation plots [from Bayesian models](https://easystats.github.io/see/articles/bayestestR.html).
