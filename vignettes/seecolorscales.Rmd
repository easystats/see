...
In this vignettes, we show the default palettes for the different color scales
...
d <- data.frame(x = names(flat_colors()))
...
d <- data.frame(
  x = names(okabeito_colors())
)

p8 <- ggplot(d, aes(x, fill = x)) +
  geom_bar(width = 0.9) +
  scale_fill_manual(values = okabeito_colors()) +
  scale_y_continuous(labels = NULL, expand = c(0, 0)) +
  guides(fill = "none") +
  ylab(NULL) +
  xlab(NULL) +
...
p9 <- ggplot(d, aes(x, fill = x)) +
  geom_bar(width = 0.9) +
  scale_fill_manual(values = uke_colors()) +
  scale_y_continuous(labels = NULL, expand = c(0, 0)) +
  guides(fill = "none") +
  ylab(NULL) +
  xlab(NULL) +
...
plots(
  ...
  n_columns = 1L,
  tags = c("Social", "Flat", "Material", "Metro", "See", "Pizza", "Bluebrown", "Okabe-Ito", "UKE")
)
```
