...
The grammar of graphics [@wilkinson2012grammar], through its implementation in the *ggplot2* [@Wickham2016] package, has become widely popular in R, and a number of add-ons packages devoted to data visualization were built to graphically exploring statistical models^[For a sampling of these packages, see: <https://exts.ggplot2.tidyverse.org/gallery/>]. A few of these packages provide ready-made plots or geometric layers for conveniently preparing common visualizations, but without linking them to any particular statistical analyses (e.g., *ggrepel*, *ggalluvial*, *ggridges*, *ggdist*, etc.). A few other packages *do* provide visualizations linked to particular statistical analyses (e.g., *ggpubr*, *tidymv*, *ggstatsplot*, *survminer*, etc.), but they tend to be more specialized. For example, the *ggstatsplot* package [@Patil2021] offers visualizations only for statistical analysis of one-way factorial designs, and the *plotmm* package [@Waggoner2020] aims at supporting only specific types of mixture model objects. 

The *see* package, on the other hand, is designed to work with objects created by the other *easystats* packages, such as parameters tables, performance tables, correlation matrices, etc., which themselves support a wide range of statistical models. As such, visualizations corresponding to all stages of statistical analysis, from model fitting to reporting, can be easily created using *see*. Additionally, it also exports some useful aesthetic utilities to further embellish users' plots. The result is a convenient package that minimizes the barrier to producing high-quality statistical visualizations in R.


# Organization and API

The reason why visualisation capabilities are included into an independent package (instead of being weaved within each package alongside the main function) is first and foremost to address a dependency managing issue. [Talk about ggplot dependency and the "optional dependency" system in the other packages]

The central ethos underlying *easystats* is to make things easy for the user, which involves amongst other an intuitive and consistent syntax, consistent and transparent argument names, comprehensive documentation, informative warnings and error messages, and smart functions with sensible default parameters. The *see* package follows this philosophy by *not* introducing any new function for most of its features. Indeed, is typically only implements a `plot()` method that can be run on an object created by an *easystats* function. These plots are further fully customizable by adding *ggplot* geoms in a traditional way (e.g., adding `ggtitle()` to customize the title of a plot).
