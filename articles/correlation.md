...
[`library`](https://rdrr.io/r/base/library.html)`(`[`correlation`](https://easystats.github.io/correlation/)`)`` `[`library`](https://rdrr.io/r/base/library.html)`(`[`see`](https://easystats.github.io/see/)`)`` `[`library`](https://rdrr.io/r/base/library.html)`(`[`ggplot2`](https://ggplot2.tidyverse.org)`)`
...
`result`` ``<-`` `[`cor_test`](https://easystats.github.io/correlation/reference/cor_test.html)`(``iris``, ``"Sepal.Length"``, ``"Petal.Width"``)`` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``result``)`

![](correlation_files/figure-html/unnamed-chunk-3-1.png)

We can even customize that to make it more beautiful:

[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``result``,`` `` point ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` aes ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``color ``=`` ``"Petal.Width"``, size ``=`` ``"Sepal.Length"``)``,`` `` alpha ``=`` ``0.66`` `` ``)``,`` `` smooth ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``color ``=`` ``"black"``, se ``=`` ``FALSE``)`` ``)`` ``+`` `` ``see``::`[`theme_modern`](https://easystats.github.io/see/reference/theme_modern.md)`(``)`` ``+`` `` ``see``::`[`scale_color_material_c`](https://easystats.github.io/see/reference/scale_color_material.md)`(``palette ``=`` ``"rainbow"``, guide ``=`` ``"none"``)`` ``+`` `` `[`scale_size_continuous`](https://ggplot2.tidyverse.org/reference/scale_size.html)`(``guide ``=`` ``"none"``)`
...
`result`` ``<-`` `[`correlation`](https://easystats.github.io/correlation/reference/correlation.html)`(``iris``)`` `` ``result`` ``#> # Correlation Matrix (pearson-method)`` ``#> `` ``#> Parameter1 | Parameter2 | r | 95% CI | t(148) | p`` ``#> -------------------------------------------------------------------------`` ``#> Sepal.Length | Sepal.Width | -0.12 | [-0.27, 0.04] | -1.44 | 0.152 `` ``#> Sepal.Length | Petal.Length | 0.87 | [ 0.83, 0.91] | 21.65 | < .001***`` ``#> Sepal.Length | Petal.Width | 0.82 | [ 0.76, 0.86] | 17.30 | < .001***`` ``#> Sepal.Width | Petal.Length | -0.43 | [-0.55, -0.29] | -5.77 | < .001***`` ``#> Sepal.Width | Petal.Width | -0.37 | [-0.50, -0.22] | -4.79 | < .001***`` ``#> Petal.Length | Petal.Width | 0.96 | [ 0.95, 0.97] | 43.39 | < .001***`` ``#> `` ``#> p-value adjustment method: Holm (1979)`` ``#> Observations: 150`` `` `[`summary`](https://rdrr.io/r/base/summary.html)`(``result``)`` ``#> # Correlation Matrix (pearson-method)`` ``#> `` ``#> Parameter | Petal.Width | Petal.Length | Sepal.Width`` ``#> -------------------------------------------------------`` ``#> Sepal.Length | 0.82*** | 0.87*** | -0.12`` ``#> Sepal.Width | -0.37*** | -0.43*** | `` ``#> Petal.Length | 0.96*** | | `` ``#> `` ``#> p-value adjustment method: Holm (1979)`

The result from [`summary()`](https://rdrr.io/r/base/summary.html) can
be used to create a plot.

`s`` ``<-`` `[`summary`](https://rdrr.io/r/base/summary.html)`(``result``)`` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``s``)`
...
`s`` ``<-`` `[`summary`](https://rdrr.io/r/base/summary.html)`(``result``, redundant ``=`` ``TRUE``)`` `` ``s`` ``#> # Correlation Matrix (pearson-method)`` ``#> `` ``#> Parameter | Sepal.Length | Sepal.Width | Petal.Length | Petal.Width`` ``#> ----------------------------------------------------------------------`` ``#> Sepal.Length | | -0.12 | 0.87*** | 0.82***`` ``#> Sepal.Width | -0.12 | | -0.43*** | -0.37***`` ``#> Petal.Length | 0.87*** | -0.43*** | | 0.96***`` ``#> Petal.Width | 0.82*** | -0.37*** | 0.96*** | `` ``#> `` ``#> p-value adjustment method: Holm (1979)`` `` `[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(``s``)`
...
[`plot`](https://rdrr.io/r/graphics/plot.default.html)`(`[`summary`](https://rdrr.io/r/base/summary.html)`(`[`correlation`](https://easystats.github.io/correlation/reference/correlation.html)`(`` `` data ``=`` ``mtcars``[`[`c`](https://rdrr.io/r/base/c.html)`(``"wt"``, ``"mpg"``, ``"drat"``)``]``,`` `` rename ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"weight"``, ``"miles per gallon"``, ``"rear axle ratio"``)`` ``)``)``)`
