...
The palette based on [material design colors](https://materialui.co/).
Use `scale_color_material_d()` for *discrete* categories and
`scale_color_material_c()` for a *continuous* scale, or use the
`discrete` argument in `scale_color_material()`.
